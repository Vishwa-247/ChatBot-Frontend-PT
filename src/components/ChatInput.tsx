...
export default function ChatInput({
  ...
  useEffect(() => {
    if (rewriteMessage) {
      setIsRewriting(true);
      setInput(
        `Please rewrite the following response to make it better, more accurate, or more helpful:\n\n"${rewriteMessage}"\n\nRewritten version:`
      );
      ...
      try {
        const processedContents = await Promise.all(
          files.map(async (file) => {
            try {
              const result = await FileProcessor.processFile(file, chatId);
      ...
      if (documentFiles.length > 0) {
        ...
        for (const file of documentFiles) {
          try {
            ...
            // Add chat_id if available, otherwise use a default
            if (chatId) {
              formData.append("chat_id", chatId);
            } else {
              formData.append("chat_id", "default_chat");
            }
  ...
  const handleUrlScraping = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "URL content has been saved to your library. Check the preview to send it to AI.",
      });
    ...
    <>
      <form onSubmit={handleSubmit} className="p-4 border-t border-border/30">
        <div className="relative max-w-3xl mx-auto">
          ...
          <div className="relative">
            <Textarea
              ...
              placeholder={
                isRewriting
                  ? "Edit the rewrite request above..."
            ...
            <div className="absolute right-2 bottom-1.5 flex gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem
                    onClick={() => triggerFileInput(".pdf,.docx,.txt,.md")}
                  >
        ...
        <div className="text-xs text-center mt-2 text-muted-foreground">
          {isRewriting
            ? "Rewriting mode - Edit the prompt above and send"
            : processingFiles
            ? "Processing files..."
            : uploadingDocument
            ? "Processing document for AI context..."
            : ""}
        </div>
      </form>
