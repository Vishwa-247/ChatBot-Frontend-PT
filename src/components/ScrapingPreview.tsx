...
import {
  ScrapedContent,
  ScrapingStorageService,
} from "@/services/scrapingStorageService";
...
export default function ScrapingPreview({
  open,
  onOpenChange,
  onSendToAI,
}: ScrapingPreviewProps) {
  const [scrapedContent, setScrapedContent] = useState<ScrapedContent[]>([]);
  const [selectedContent, setSelectedContent] = useState<ScrapedContent | null>(
    null
  );
  const [previewOpen, setPreviewOpen] = useState(false);
