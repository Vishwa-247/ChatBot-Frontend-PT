...
export class FileProcessor {
  ...
  static async extractTextFromImage(
    ...
    try {
      ...
      await this.storeOCRInKnowledgeBase(file.name, text, chatId);
  ...
  static async storeOCRInKnowledgeBase(
    filename: string,
    ocrText: string,
    chatId?: string
  ): Promise<void> {
    try {
      ...
      formData.append(
        "file",
        blob,
        `ocr_${filename.replace(/\.[^/.]+$/, "")}.txt`
      );

      // Add chat_id if provided, otherwise use a default
      if (chatId) {
        formData.append("chat_id", chatId);
      } else {
        // Use a default chat_id for OCR documents
        formData.append("chat_id", "ocr_documents");
      }
  ...
  static async processFile(
    file: File,
    chatId?: string
  ): Promise<{ type: string; content: string; showContent: boolean }> {
    if (file.type.startsWith("image/")) {
      const text = await this.extractTextFromImage(file, chatId);
